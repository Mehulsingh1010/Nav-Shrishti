...
import { Package, ShoppingBag, Calendar, Clock, ArrowRight } from "lucide-react";
      ...
      {ordersWithItems.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-xl font-medium text-gray-900 mb-2">कोई खरीद नहीं मिली</h3>
            <p className="text-gray-500 mb-6">आपने अभी तक कोई उत्पाद नहीं खरीदा है।</p>
            <Link href="/products">
              <Button className="bg-orange-600 hover:bg-orange-700">उत्पाद ब्राउज़ करें</Button>
            </Link>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={order.id} className="overflow-hidden">
              <CardHeader className="bg-orange-50 border-b border-orange-100">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div>
                    <CardTitle className="text-lg text-orange-800">ऑर्डर #{order.id}</CardTitle>
                    <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                      <Calendar className="h-4 w-4 mr-1" /> {formatDate(order.createdAt)}
                      <Clock className="h-4 w-4 mr-1" /> {new Date(order.createdAt).toLocaleTimeString("hi-IN")}
                    </div>
                  </div>
...
/>
                    <div className="flex-grow ml-4">
                      ...
                      <span className="font-semibold block">{formatPrice(item.quantity * item.pricePerUnit)}</span>
                    </div>
                  </div>
                ))}
                <div className="p-4 bg-gray-50 flex justify-between">
                  <span className="text-sm text-gray-500">भुगतान स्थिति: {order.payment?.status === "captured" ? "सफल" : "लंबित"}</span>
                  <Link href={`/user-dashboard/purchase/order/${order.id}`}>
                    <Button variant="ghost" className="text-orange-600 hover:text-orange-700">विवरण देखें <ArrowRight className="ml-1 h-4 w-4" /></Button>
                  </Link>
                </div>
              </CardContent>
            </Card>
